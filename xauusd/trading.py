    ...
    for pos in positions:
        ...
        if r_multiple >= partial_trigger_rr:
            ...
            if close_result and close_result.retcode == mt5.TRADE_RETCODE_DONE:
                ...
                log_notify(f"[XAUUSD PARTIAL PROFIT] Closed {partial_close_percent}% of {direction} | "
                ...
                if partial_move_to_breakeven and remaining_volume > 0:
                    ...
                    if modify_result and modify_result.retcode == mt5.TRADE_RETCODE_DONE:
                        log_notify(f"[XAUUSD BREAKEVEN] SL moved to {new_sl:.2f}")
...
def check_smart_exit(symbol, positions, tracked_positions):
    ...
    for pos in positions:
        ...
        if should_close:
            ...
            if close_result and close_result.retcode == mt5.TRADE_RETCODE_DONE:
                direction = "BUY" if pos.type == 0 else "SELL"
                log_notify(f"[XAUUSD SMART EXIT] {direction} closed | Reason: {close_reason} | "
...
def shutdown_handler(signum, frame):
    ...
    log_notify(f"[XAUUSD SHUTDOWN] Bot stopped by signal {sig_name} (PID: {os.getpid()})")
    mt5.shutdown()
    sys.exit(0)
...
try:
    ...
    log_notify(f"[XAUUSD STARTUP] Bot started (PID: {os.getpid()}) | Strategy: {strategy} | Symbol: {symbol} | Balance: ${startup_balance:.2f}")
    log_notify(f"[XAUUSD STARTUP] Config: SL={sl_pips}, TP={tp_pips}, Timeframe={timeframe_str}, Higher TF={higher_timeframe_str}")
    if max_spread_percent > 0 or loss_cooldown_minutes > 0 or max_consecutive_losses > 0:
        log_notify(f"[XAUUSD STARTUP] Defensive: MaxSpread={max_spread_percent}%, Cooldown={loss_cooldown_minutes}min, MaxConsecLoss={max_consecutive_losses}")
    if enable_ema_trend_filter:
        log_notify(f"[XAUUSD STARTUP] EMA Trend Filter: ENABLED (EMA{ema_fast_period}/EMA{ema_slow_period}) - BUY blocked in downtrend")
    if dynamic_sizing_enabled:
        log_notify(f"[XAUUSD STARTUP] Position Sizing: Dynamic ({risk_percent}% risk), Lot range: {min_lot_size}-{max_lot_size}")
    else:
        log_notify(f"[XAUUSD STARTUP] Position Sizing: Fixed lot = {lot}")
    if session_trading_enabled:
        log_notify(f"[XAUUSD STARTUP] Session Trading: ENABLED - Off-hours trading {'ON' if session_definitions.get('off_hours', {}).get('enabled', False) else 'OFF'}")
    if enable_partial_profit:
        log_notify(f"[XAUUSD STARTUP] Partial Profit: ENABLED - Close {partial_close_percent}% at {partial_trigger_rr}R, breakeven SL")
                ...
                if len(row) >= 5 and today_str in row[0]:
                    try:
                        ...
                        if pl > 0:
                            ...
                            total_wins += 1  # breakeven = win
                            consecutive_losses = 0
                    except (ValueError, IndexError):
                        pass
        if daily_trade_count > 0:
            wr = total_wins / (total_wins + total_losses) * 100 if (total_wins + total_losses) > 0 else 0
            log_only(f"[XAUUSD RESTORE] Restored today's stats from CSV: {total_wins}W/{total_losses}L ({wr:.0f}% WR) | P/L: ${daily_pl:.2f} | Trades: {daily_trade_count}")
    except FileNotFoundError:
        pass
    except Exception as e:
        log_only(f"[XAUUSD RESTORE] Failed to restore stats: {e}")

    # Defensive trading tracking (already restored above)
    # consecutive_losses and last_loss_time set during restore
    ...
    def log_trade_result(direction, entry_price, exit_price, profit, confidence=None, close_reason=""):
        ...
        trade_msg = f"[XAUUSD TRADE {result}] {direction}{reason_str} | Entry: {entry_price:.2f} | Exit: {exit_price:.2f} | P/L: ${profit:.2f}{conf_str}"
        stats_msg = f"[XAUUSD STATS] Wins: {total_wins} | Losses: {total_losses} | Win Rate: {win_rate:.1f}% | Session P/L: ${cumulative_pl:.2f}"
        ...
        if max_consecutive_losses > 0 and consecutive_losses >= max_consecutive_losses:
            log_notify(f"[XAUUSD ALERT] {consecutive_losses} consecutive losses! Trading paused for 1 hour.")
            ...
            if closing_deal:
                ...
                if profit == 0.0 and exit_price != pos_info['entry_price']:
                    ...
                    if symbol_info:
                        ...
                        log_notify(f"[XAUUSD P/L FIX] MT5 reported $0, recalculated from prices: ${profit:.2f}")
                ...
                log_notify(f"[XAUUSD WARN] Position {ticket} ({pos_info['direction']}) closed but no exit deal found after 3 retries")
    ...
    while True:
        ...
        if is_daily_training_time() and (last_training_date != today_date) and not _training_in_progress:
            log_notify(f"[XAUUSD AUTOMATION] Daily ML training time (8am ET). Starting background training...")
            ...
            def _run_background_training():
                ...
                try:
                    ...
                    if result.returncode != 0:
                        log_notify(f"[XAUUSD AUTOMATION] train_ml_model.py failed: {result.stderr[:500]}")
                    else:
                        log_notify("[XAUUSD AUTOMATION] ML model training completed. Will reload on next cycle.")
                        _training_needs_reload = True
                except subprocess.TimeoutExpired:
                    log_notify("[XAUUSD AUTOMATION] ML training timed out after 10 minutes")
                except Exception as e:
                    log_notify(f"[XAUUSD AUTOMATION] Background training error: {e}")
                finally:
        ...
        if _training_needs_reload and not _training_in_progress:
            try:
                if ml_predictor is not None:
                    ml_predictor.load_model()
                log_notify(f"[XAUUSD AUTOMATION] ML model reloaded successfully after background training")
            except Exception as e:
                log_notify(f"[XAUUSD AUTOMATION] Failed to reload ML model: {e}")
        ...
        if not market_open:
            log_only(f"[XAUUSD HEARTBEAT] Market is closed for {symbol}. Bot is running. UTC: {now.isoformat()}, ET: {now_et.isoformat()}")
            time.sleep(60)
            continue
        ...
        if last_heartbeat_hour != current_hour:
            ...
            log_notify(f"[XAUUSD HEARTBEAT] Bot running. Balance: ${balance:.2f} | Strategy: {strategy} | {symbol}")
        ...
        if trade_signal is not None and max_consecutive_losses > 0 and consecutive_losses >= max_consecutive_losses:
            msg = f"[XAUUSD CIRCUIT BREAKER] {consecutive_losses} consecutive losses - trading paused"
            if last_filter_message != msg:
                log_only(msg)
                last_filter_message = msg
            ...
            if last_loss_time is not None:
                ...
                if hours_since_loss >= 1:
                    consecutive_losses = 0
                    log_notify(f"[XAUUSD CIRCUIT BREAKER] Reset after 1 hour cooldown. Trading resumed.")
            trade_signal = None
        ...
        if trade_signal == "buy":
            if position_type is None:
                ...
                if result and result.retcode == mt5.TRADE_RETCODE_DONE:
                    log_notify(f"[XAUUSD BUY] order placed, ticket: {result.order}, price: {ask}")
                    ...
                    log_notify(f"[XAUUSD BALANCE] Account balance after BUY: ${balance:.2f}")
            ...
            if position_type is None:
                ...
                result = mt5.order_send(request)
                if result and result.retcode == mt5.TRADE_RETCODE_DONE:
                    log_notify(f"[XAUUSD SELL] order placed, ticket: {result.order}, price: {bid}")
                    # Track this position for SL/TP detection
                    ml_conf = last_trade_confidence if strategy == "ml_xgboost" else None
                    tracked_positions[result.order] = {
                        'direction': 'SELL',
                    ...
                    balance = mt5.account_info().balance
                    log_notify(f"[XAUUSD BALANCE] Account balance after SELL: ${balance:.2f}")
                    daily_trade_count += 1  # Increment daily trade counter
                elif result and result.retcode == 10031:
                    # Error 10031 = "No changes" - likely already have a position (never notify)
                ...
                for pos in positions:
                    ...
                    if pos.type == mt5.POSITION_TYPE_BUY:
                        ...
                        if (pos.sl is None or pos.sl == 0) or (new_sl > pos.sl):
                            ...
                            modify_result = mt5.order_send(modify_request)
                            if modify_result and modify_result.retcode == mt5.TRADE_RETCODE_DONE:
                                log_notify(f"[XAUUSD TRAILING SL] BUY position {pos.ticket}: SL updated to {new_sl:.5f} (ATR trailing)")
    ...
    log_notify(f"[XAUUSD SHUTDOWN] Bot crashed with error: {e} (PID: {os.getpid()})")
    raise
finally:
    try:
        account = mt5.account_info()
        final_balance = account.balance if account else 0
        log_notify(f"[XAUUSD SHUTDOWN] Bot shutting down (PID: {os.getpid()}) | Final Balance: ${final_balance:.2f}")
    except Exception:
        log_only(f"[XAUUSD SHUTDOWN] Bot shutting down (PID: {os.getpid()}) | Balance unavailable")
    mt5.shutdown()
