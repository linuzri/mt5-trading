...
try:
    ...
    try:
        ...
        if daily_trade_count > 0:
            ...
            log_only(f"[EURUSD RESTORE] Restored today's stats from CSV: {total_wins}W/{total_losses}L ({wr:.0f}% WR) | P/L: ${daily_pl:.2f} | Trades: {daily_trade_count}")
    except FileNotFoundError:
        pass
    except Exception as e:
        log_only(f"[EURUSD RESTORE] Failed to restore stats: {e}")
